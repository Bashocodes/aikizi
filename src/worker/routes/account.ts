...
export async function ensureAccount(env: Env, req: Request) {
  ...
  if (!authJwt) return bad('auth required', 401);
  ...
  if (!user_id) {
    ...
    user_id = inserted.id;
    await dbClient.from('entitlements').insert({ user_id, monthly_quota: 1000, tokens_balance: 1000, last_reset_at: new Date().toISOString(), next_reset_at: new Date(Date.now()+30*24*3600*1000).toISOString() });
  }
